...
export class CreateVisualizer {
	...
	private readonly container: Attachment;

	constructor(parent?: Instance) {
		/** Creating the beam instance. */
	...
	public MoveTo(startPoint: Vector3, directionOrEndPoint: Vector3, length?: number): void {
		if (length !== undefined) {
			// If length is provided, calculate the end point using the direction
			// and length
			/** Normalize the direction. */
			const direction = directionOrEndPoint.Unit;
			/** Default length logic. */
			const endPoint = startPoint.add(direction.mul(length));
